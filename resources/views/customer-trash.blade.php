                        ...
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <div class="flex space-x-2">
                                ...
                                <a href="{{ route('customer.delete', ['id' => $custo->customer_id]) }}"
                                    class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold py-1 px-2 rounded"
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            @foreach ($customer as $custo)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="px-4 py-3 max-w-xs truncate" title="{{ $custo->address }}">
                                        {{ $custo->address ?: 'N/A' }}</td>
                                    ...
                                    <td class="px-4 py-3">
                                        @if ($custo->gender == 'M')
                                            <span
                                                class="inline-block px-2 py-1 rounded text-xs font-semibold bg-blue-100 text-blue-800">
                                                Male
                                            </span>
                                        @elseif ($custo->gender == 'F')
                                            <span
                                                class="inline-block px-2 py-1 rounded text-xs font-semibold bg-pink-100 text-pink-800">
                                                Female
                                            </span>
                                        @else
                                            <span
                                                class="inline-block px-2 py-1 rounded text-xs font-semibold bg-gray-100 text-gray-800">
                                                Other
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('customer.force-delete', ['id' => $custo->customer_id]) }}"
                                                class="bg-red-600 hover:bg-red-700 text-white font-bold py-1 px-2 rounded text-xs transition duration-200"
                                                onclick="return confirm('Are you sure you want to delete this customer?')">
                                                Delete
                                            </a>
                                            <a href="{{ route('customer.restore', ['id' => $custo->customer_id]) }}"
                                                class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-xs transition duration-200">
                                                Restore
                                            </a>
                                        </div>
                                    </td>
